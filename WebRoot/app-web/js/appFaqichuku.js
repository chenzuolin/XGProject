...
function yanzhen(){
	...
	if(!reg.test($("#yunType").val())) {
		alert("运输方式必须为汽运或火运！");
		return false;
	}
	if($("#chehao").val() == "" && $("#peisong").prop("checked")==false) {
		alert("车号有误！");
		return false;
	}
	
	if($("#sijiName").val()=="" && $("#peisong").prop("checked")==false) {
		alert("司机姓名不能为空！");
		return false;
	}
	
	if($("#danweiTel").val()=="" && $("#peisong").prop("checked")==true ) {
		alert("联系电话号码不能为空！");
		return false;
	}
	if(telyanzheng.test($("#danweiTel").val())==false && $("#peisong").prop("checked")==true ) {
		alert("联系电话有误，必须11位！");
		return false;
	}
		
	if(!telyanzheng.test($("#tel").val()) && $("#peisong").prop("checked")==false) {
		alert("司机电话号码有误！");
		return false;
	}
