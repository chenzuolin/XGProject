					...
					function() {
						...
						for ( var i = 0; i < len; i++) {
							...
							if ($(
								...
								var zhongliang = $(
										".content_middle table tbody tr").eq(i)
										.children("td").eq(9).children("input")
										.val();
								var jianshu = $(
										".content_middle table tbody tr").eq(i)
								...
								$(".content_middle table tbody")
										.append(
												"<tr><td><input type='checkbox' value='"
														...
														+ obj[i].zhongliang
														+ "' name='shiftWeights' style='display:none;'/></td><td>"
														+ obj[i].jianshu
														+ "<input type='checkbox' value='"
														+ obj[i].jianshu
														+ "' name='shiftNumbers' style='display:none;'/></td><td><input type='text' name='r' class='remark'></td></tr>");
							}
...
function zhixingren() {
	$(function() {
		$.ajax({
			type : "post",
			url : "shift.do?flag=getQueryData&ff=zhixing&time="
					+ new Date().getTime(),
			async : true,
			dataType : "json",
			success : function(obj) {
