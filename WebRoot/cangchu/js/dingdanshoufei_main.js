...
function jiazaiLoad() {
	...
	if ($("#type_xuanze").val() == "出库订单") {
		...
		selectShoufeiPlanExport();

	} else if ($("#type_xuanze").val() == "过户订单") {
		// 查询正在审核的过户订单
		guohushenhe();
	} else {// 进入库方法
		selectShoufeiPlanInput();
	}
...
function selectShoufeiPlanInput() {
	...
	$
			.ajax({
				type : "post",
				url : "inputOperate.do?flag=selectShoufeiInputAjax",
				data : {
					"pageNow" : pageNow
				},// 传入当前页
				dataType : "json",
											...
											$(".content_center")
													.append(
															"<div class='content_tab' title='开始作业'>"
																	...
																	+ "<td colspan='3' class='tr_border_right_xia' title="
																	+ item.beizhu
																	+ "><span class='span' style='display:block;width: 200px;'>"
																	+ item.beizhu
																	+ "</span></td>"
																	+ "</tr>" +
...
function selectShoufeiPlanExport() {
	...
	$
			.ajax({
				type : "post",
				url : "exportOperate.do?flag=getShoufeiChaxun",
				data : {
					"danhao" : $kehudanhao,
					"huozhu" : $huozhu,
					"pageNow" : pageNow
...
function sousu_but() {
	...
	$
			.ajax({
				type : "post",
				url : "inputOperate.do?flag=selectShoufeiInputAjaxTj",
				data : {
					"danhao" : $kehudanhao,
					"huozhu" : $huozhu,
					"pageNow" : pageNowTow
											...
											$(".content_center")
													.append(
															"<div class='content_tab' title='开始作业'>"
																	...
																	+ "</tr>"
																	+ "<tr class='tr_bg'>"
																	+ "<td align='right' class='tr_border_left'><strong>货物品类：</strong></td>"
																	+ "<td><span>"
																	+ item.goodsCategoryName
																	+ "</span></td>"
																	...
																	+ "<td class='tr_border_right'><span>"
																	+ item.goodsQualityName
																	+ "</span></td>"
																	+ "</tr>"
																	+ "<tr class='tr_bg'>"
																	+ "<td align='right' class='tr_border_left_xia'><strong>属性：</strong></td>"
																	+ "<td><span>"
																	+ item.goodsPropertyName
																	+ "</span></td>"
																	...
																	+ "<td class='tr_border_right_xia'><span>"
																	+ item.inputCreateTime
																	+ "</span></td>"
																	+ "</tr>"
																	+ "<tr class='tr_diaodu_bg'>"
																	+ "<td align='right' class='tr_border_left'><strong>调度员：</strong></td>"
																	+ "<td><span>"
																	+ item.diaodu
																	+ "</span></td>"
																	+ "<td align='right'><strong>分配时间：</strong></td>"
																	+ "<td><span>"
																	+ item.fenpeiTime
																	+ "</span></td>"
																	+ "<td align='right'><strong>过磅/理算：</strong></td>"
																	+ "<td><span>"
																	+ item.guoLi
																	+ "</span></td>"
																	+ "<td align='right'><strong>分配重量：</strong></td>"
																	+ "<td class='tr_border_right'><span>"
																	+ item.feipeiWeight
																	+ "</span></td>"
																	+ "</tr>"
																	+ "<tr class='tr_diaodu_bg'>"
																	+ "<td align='right' class='tr_border_left_xia'><strong>库位：</strong></td>"
																	+ "<td><span>"
																	+ item.kuwei
																	+ "</span></td>"
																	+ "<td align='right'><strong>保管员：</strong></td>"
																	+ "<td><span>"
																	+ item.baoguan
																	+ "</span></td>"
...
function guohushenhe() {
	...
	$
			.ajax({
				type : "post",
				url : "shiftStockSeed.do?flag=getTongGuoShiftStock",
				data : {
					"pageNow" : page,
					"tihuohao" : kehudanhao,
					"huozhu" : huozhu
											...
											$(".content_center")
													.append(
															"<div class='content_tab' title='开始作业'>"
																	...
																	+ item.zhongliang
																	+ "吨</span></td>"
																	+ "<td align='right' onclick='tiaozhuanguohu(this)'><strong>应收费用：</strong></td>"
