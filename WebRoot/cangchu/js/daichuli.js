...
function chukuDaichuli() {
	$(function() {
		...
		$
				.ajax({
					type : "post",
					url : "exportSeed.do?flag=getDaiChuLi",
					data : {
						"pageNow" : pageNow,
						"kehubianhao" : kehuhao,
						"jiancheng" : huozhu
										...
										function(i, item) {
											...
											$div
													.append("<table cellpadding='0' cellspacing='0'>"
															...
															+ item.sijitel
															+ "</span></td>"
															+ "<td colspan='2' rowspan='4'><a class='aClass' onclick='tianzhuanchuku(this)' style='cursor:pointer;' >详情"
...
function selectPlanInputChange() {
	...
	if ($("#type_xuanze").val() == "出库订单") {
		...
		$
				.ajax({// ajax提交方式
					type : "post",
					url : "input.do?flag=selectPlanInputAjax",
					data : {
						"pageNow" : pageNow
					},
					dataType : "json",
					success : function(inputseed) {
						if (inputseed.qingkong == "clean") {
							alert("请求数据已完！");
						}
											...
											function(i, item) {
												...
												$div
														.append("<table cellpadding='0' cellspacing='0'>"
																+ "<tbody> "
																+ "<tr class='zhankai'>"
																+ "<td align='right' width='100px'><strong>订单编号：</strong></td>"
																+ "<td width='130px'>"
																+ "<span>"
																+ item.zongId
																+ "</span>"
																...
																+ "' />"
																+ "</td>"
																+ "<td align='right' width='100px'><strong>货主：</strong></td>"
																+ "<td width='160px'><span>"
																+ item.clientLoginName
																+ "</span></td>"
																+ "<td align='right' width='100px'><strong>客户单号：</strong></td>"
																+ "<td width='130px'><span>"
																+ item.inputClientNumber
																+ "</span></td>"
																+ "<td align='right' width='100px'><strong>有效期：</strong></td>"
																+ "<td width='100px'><span>"
																+ item.inputDefinedOne
																+ "</span></td>"
																+ "<td align='right' width='100px'><strong>订单状态：</strong></td>"
																+ "<td width='130px'><span style='color:red;'>"
																+ item.status
																+ "</span></td>"
																+ "<td rowspan='3' valign='top' width='50px'><img src='cangchu/img/xiangxia.png' width='20' class='zhankai' style='display:block;margin-top:10px;' /></td>"
																+ "</tr>"
																+ "<tr>"
																+ "<td onclick='tanchuAdd(this)' align='right'><strong>运输方式：</strong></td>"
																+ "<td onclick='tanchuAdd(this)'><span>"
																+ item.inputCarryType
																+ "</span></td>"
																+ "<td onclick='tanchuAdd(this)' align='right'><strong>车号：</strong></td>"
																+ "<td onclick='tanchuAdd(this)'><span>"
																+ item.inputPlateNumber
																+ "</span></td>"
																+ "<td onclick='tanchuAdd(this)' align='right'><strong>司机姓名：</strong></td>"
																+ "<td onclick='tanchuAdd(this)'><span>"
																+ item.inputDriverName
																+ "</span></td>"
																+ "<td onclick='tanchuAdd(this)' align='right'><strong>司机电话：</strong></td>"
																+ "<td onclick='tanchuAdd(this)'><span>"
																+ item.inputDriverTel
																+ "</span></td>"
																+ "<td colspan='2' rowspan='4'><a class='aClass' href='inputOperate.do?flag=getAllFinally&ziId="
																+ item.iseedId
																+ "' >详情</a></td>"
																+ "</tr>"
																+ "<tr class='tr_bg'>"
																+ "<td onclick='tanchuAdd(this)' align='right' class='tr_border_left'><strong>货物品类：</strong></td>"
																+ "<td onclick='tanchuAdd(this)'><span>"
																+ item.goodsCategoryName
																+ "</span></td>"
																+ "<td onclick='tanchuAdd(this)' align='right'><strong>名称：</strong></td>"
																+ "<td onclick='tanchuAdd(this)'><span>"
																+ item.goodsName
																+ "</span></td>"
																+ "<td onclick='tanchuAdd(this)' align='right'><strong>规格：</strong></td>"
																+ "<td onclick='tanchuAdd(this)'><span>"
																+ item.goodsStandardName
																+ "</span></td>"
																+ "<td onclick='tanchuAdd(this)' align='right'><strong>材质：</strong></td>"
																+ "<td onclick='tanchuAdd(this)' class='tr_border_right'><span>"
																+ item.goodsQualityName
																+ "</span></td>"
																+ "</tr>"
																+ "<tr class='tr_bg'>"
																+ "<td onclick='tanchuAdd(this)' align='right' class='tr_border_left_xia'><strong>属性：</strong></td>"
																+ "<td onclick='tanchuAdd(this)'><span>"
																+ item.goodsPropertyName
																+ "</span></td>"
																+ "<td onclick='tanchuAdd(this)' align='right'><strong>产地：</strong></td>"
																+ "<td onclick='tanchuAdd(this)'><span>"
																+ item.goodsYieldlyName
																+ "</span></td>"
																+ "<td onclick='tanchuAdd(this)' align='right'><strong>重量：</strong></td>"
																+ "<td onclick='tanchuAdd(this)'><span>"
																+ item.iseedShouldWeight
																+ "</span></td>"
																+ "<td onclick='tanchuAdd(this)' align='right'><strong>发起时间：</strong></td>"
																+ "<td onclick='tanchuAdd(this)' class='tr_border_right_xia' width='200px'><span>"
...
function sousu_but() {
	...
	if ($("#type_xuanze").val() == "出库订单") {
		...
		$
				.ajax({// ajax提交方式
					type : "post",
					url : "input.do?flag=selectPlanInputDZAjax",
					data : {
						"danhao" : $kehudanhao,
						"huozhu" : $huozhu,
						"pageNow" : pageNowTow
											...
											function(i, item) {
												...
												$div
														.append("<table cellpadding='0' cellspacing='0'>"
																...
																+ "</span></td>"
																+ "<td onclick='tanchuAdd(this)' align='right'><strong>发起时间：</strong></td>"
																+ "<td onclick='tanchuAdd(this)' class='tr_border_right_xia'><span>"
																+ item.inputCreateTime
																+ "</span></td>"
																+ "</tr>"
																+
	...
	$("#daichuli_chuku_tijiao")
			.click(
					function() {
						...
						if (parseFloat(yczl) <= parseFloat(sczl)+parseFloat(fpzl)) {
							if (confirm("该订单已出库重量"+parseFloat(sczl)+"吨，是否继续分配") == false) {
								return false;
							}
						}
						/*
