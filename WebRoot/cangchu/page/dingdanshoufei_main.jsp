...
<script src="cangchu/js/dingdanshoufei_main.js"></script>
...
<script>
	$(function() {
		...
		/*$(".content_tab table tr td").not("td:last-child").click(function() {
			...
			if(confirm("确定要开始作业吗？")) {
				document.location.href = "dingdanshoufei.html";
			}
		});
		 */
	...
	function tiaozhuanruku(str) {
		$(function() {
			...
			if (confirm("确定要开始作业吗？")) {
				document.location.href = "inputOperate.do?flag=selectShoufeiOperat&caozuoid="
						+ caozuoid + "&ziId=" + ziId;
			}
		});
	}
	...
	function tiaozhuanchuku(str) {
		$(function() {
			...
			if (confirm("确定要开始作业吗？")) {
				document.location.href = "exportOperate.do?flag=getShoufeiChakai&caozuoid="
						+ caozuoid + "&ziId=" + ziId;
			}
		});
	}
	...
	function tiaozhuanguohu(str) {
		$(function() {
			...
			if (confirm("确定要开始作业吗？")) {
				...
				// id
				document.location.href = "shiftStockSeed.do?flag=getShenHeXiangXi&ff=shoufei&ssseedId="
...
<%
	...
	if(power.get(i).getFunctions().getFunctionName()!=null && power.get(i).getFunctions().getFunctionName().equals("入库收费")){
		ruku++;
	}
	if(power.get(i).getFunctions().getFunctionName()!=null && power.get(i).getFunctions().getFunctionName().equals("出库收费")){
		chuku++;
	}
	if(power.get(i).getFunctions().getFunctionName()!=null && power.get(i).getFunctions().getFunctionName().equals("过户收费")){
		guohu++;
	}
		}
		if(x==0){
		...
		<div class="daichuli_content">
						...
						<table cellpadding="0" cellspacing="0">
							<tbody>
								...
								<tr class="tr_diaodu_bg">
									...
									<td colspan="3" class="tr_border_right_xia"
										title="${operate.ioperateRemark }"><span class="span"
										style="display:block;width: 200px;">${operate.ioperateRemark
											}</span>
									</td>
								</tr>
							</tbody>
			...
			<div class="content_bottom">
				<a id="chakanGenduo" style="cursor: pointer;">查看更多</a> <input
					type="hidden" value="1" id="guohupage" />
			</div>
		</div>
		<!--daichuli_content结束-->
	</div>
