...
function queryRuKu(curr, pageRow) {
	layui.use(['element', 'table', 'laypage', 'jquery', 'layer'], function() {
		...
		var url = "/XGProject/inputSeed.do?flag=getDingdanAppTj";
		var d = {
			"startTime": begin,
			"endTime": finish,
		...
		if(data != "error") {
			...
			$.each(jdata, function(i, obj) {
				obj.xuhao = (i + 1);
				if(obj.result != null) {
					if(re.test(obj.shichuweight)) {
						shiruSum += parseFloat(obj.shichuweight);
					}
					if(re.test(obj.zhongliang)) {
...
function queryChuKu(curr, pageRow) {
	layui.use(['table', 'laypage', 'jquery', 'layer'], function() {
		...
		var url = "/XGProject/exportSeed.do?flag=getDingdanAppTj";
		var d = {
			"startTime": begin,
			"endTime": finish,
			"kehuDanhao": client_number,
...
function queryGuoHu(curr, pageRow) {
	layui.use(['element', 'table', 'laypage', 'jquery', 'layer'], function() {
		...
		var url = "/XGProject/shiftStockSeed.do?flag=getDingdanAppTj";
		var d = {
			"startTime": begin,
			"endTime": finish,
			"kehuDanhao": client_number,
		...
		if(data != "error") {
			...
			$.each(jdata, function(i, obj) {
				obj.xuhao = (i + 1);
				if(re.test(obj.zhongliang)) {
					shichuSum += parseFloat(obj.zhongliang);
				}
			});
			var sumJson = {
				"id": "当页合计：",
