   ...
   table.on('tool(demo)', function(obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
      ...
      if(layEvent === 'detail') { //点击了开始操作
         ...
         if(data.id != undefined && data.id != "" && data.id != null) {
            ...
            //转换到相应详情页面
            document.location.href = "/XGProject/shiftStockSeed.do?flag=getShenHeXiangXi&ff=shoufei&ssseedId=" + zid;
         }
         if(data.zongId != undefined && data.zongId != "" && data.zongId != null) {
            ...
            if(type == "出") {
               ...
               //跳转到相应的收费详情页面
               document.location.href = "/XGProject/exportOperate.do?flag=getShoufeiChakai&caozuoid=" + cid + "&ziId=" + zid;
            } else if(type == "入") {
               var zid = data.inputSeedId; //子订单编号
               var cid = data.inputOperateId; //操作订单编号
               //跳转到相应的收费详情界面
               document.location.href = "/XGProject/inputOperate.do?flag=selectShoufeiOperat&caozuoid=" + cid + "&ziId=" + zid;
            }
         }
      }
   });
...
function queryChuKu(curr, pageRow) {
   layui.use(['jquery', 'form', 'layer', 'table', 'laypage'], function() {
      ...
      $.ajax({
         type: "post",
         url: "/XGProject/exportOperate.do?flag=getShoufeiChaxun",
         async: true,
         data: {
            "pageNow": curr,
            "danhao": chehao,
...
function queryRuKu(curr, pageRow) {
   layui.use(['jquery', 'form', 'layer', 'table', 'laypage'], function() {
      ...
      $.ajax({
         type: "post",
         url: "/XGProject/inputOperate.do?flag=selectShoufeiInputAjaxTj",
         async: true,
         data: {
            "danhao": chehao,
            "huozhu": client,
...
function queryGuoHu(curr, pageRow) {
   layui.use(['jquery', 'form', 'layer', 'table', 'laypage'], function() {
      ...
      $.ajax({
         type: "post",
         url: "/XGProject/shiftStockSeed.do?flag=getTongGuoShiftStock",
         async: true,
         data: {
            "tihuohao": chehao,
            "huozhu": client,
