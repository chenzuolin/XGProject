   ...
   table.on('tool(demo)', function(obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
      ...
      if(layEvent === 'detail') { //点击了开始操作
         ...
         if(data.id != undefined && data.id != "" && data.id != null) {
            ...
            //转换到相应详情页面
            document.location.href = "/XGProject/exportOperate.do?flag=getGuoBangCaoZuo&eoperateId=" + encodeURI(encodeURI(cid));
         }
         if(data.zongId != undefined && data.zongId != "" && data.zongId != null) {
            ...
            document.location.href = "/XGProject/inputOperate.do?flag=selectSiBangChuli&caozuoid=" + cid;
         }
      }
   });
});
...
function queryChuKu(curr, pageRow) {
   layui.use(['jquery', 'form', 'layer', 'table', 'laypage'], function() {
      ...
      $.ajax({
         type: "post",
         url: "/XGProject/exportOperate.do?flag=getGuoBang",
         async: true,
         data: {
            "pageNow": curr,
            "kehudaohao": chehao,
...
function queryRuKu(curr, pageRow) {
   layui.use(['jquery', 'form', 'layer', 'table', 'laypage'], function() {
      ...
      $.ajax({
         type: "post",
         url: "/XGProject/inputOperate.do?flag=selectSibangInputAjaxTj",
         async: true,
         data: {
            "danhao": chehao,
            "huozhu": client,
