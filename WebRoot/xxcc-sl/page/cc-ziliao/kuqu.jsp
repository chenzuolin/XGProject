...
<html>
   ...
   <body>
      <div class="layui-fluid">
         <fieldset class="buttons">
            <legend style="font-size: 1.4rem;">库区管理</legend>
            <button class="layui-btn" id="ADD"><i class="fa fa-plus" style="padding-right: 6px;"></i>新增库区</button>
         </fieldset>
         <!--表格内容-->
         ...
         <form class="layui-form" action="">
            <div class="layui-form-item">
               ...
               <div class="layui-input-block">
                  <input type="text" name="kuqu" lay-verify="required" class="layui-input" id="kuqu1">
               </div>
            </div>
            <div class="layui-form-item">
      ...
      <div id="add" style="display: none;">
         <form class="layui-form" action="">
            <div class="layui-form-item">
               <label class="layui-form-label">库区名称：</label>
               <div class="layui-input-block">
                  <input type="text" name="kuqu" lay-verify="required" class="layui-input" id="kuqu">
               </div>
            </div>
            <div class="layui-form-item">
               <label class="layui-form-label">库区描述：</label>
   ...
   <script>
      ...
      layui.use(['table', 'form'], function() {
         ...
         $.ajax({
            type: 'post',
            url: '/XGProject/bursary.do?flag=getBursaryAll',
            async: false,
            dataType: 'json',
         ...
         table.on('tool(datatable)', function(obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            //停用操作=============================
            if(layEvent === 'del') { //删除
               layer.confirm('确定停用此库区吗？', function(index) {
                  $.ajax({
                     type: "post",
                     url: "/XGProject/bursary.do?flag=stopBursary",
                     async: false,
                     data: {
                        "id": data.id
                     },
         ...
         form.on('submit(btn1)', function(obj) {
            ...
            var datas = {
               'bursaryId': hangID,
               'bursaryName': data.kuqu,
               'bursaryDefinedTwo': data.miaoshu,
               'bursaryRemark': data.remark,
            };
            var bianji = goodsContent('/XGProject/bursary.do?flag=updateBursary', datas, 'text');
            if(bianji == 'error') {
               layer.alert('数据上传错误', {
                  icon: 2
               })
         ...
         form.on('submit(btn)', function(obj) {
            ...
            layer.confirm('确定添加库位吗！', {
               ...
               var tianjia = goodsContent('/XGProject/bursary.do?flag=saveBursary', {
                  'bursaryName': data.kuqu,
                  'bursaryDefinedTwo': data.miaoshu,
                  'bursaryRemark': data.remark,
               }, 'text');
