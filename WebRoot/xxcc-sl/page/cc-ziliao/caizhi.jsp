...
<html>
   ...
   <body>
      <div class="layui-fluid">
         <fieldset class="buttons">
            <legend>货物材质管理</legend>
            <button class="layui-btn" id="ADD"><i class="fa fa-plus" style="padding-right: 6px;"></i>新增材质</button>
         </fieldset>
         <!--表格内容-->
         ...
         <form class="layui-form" action="">
            <div class="layui-form-item">
               ...
               <div class="layui-input-block">
                  <input type="text" name="caizhi" lay-verify="required" class="layui-input" id="caizhi1">
               </div>
            </div>
            <div class="layui-form-item">
      ...
      <div id="add" style="display: none;">
         <form class="layui-form" action="">
            <div class="layui-form-item">
               <label class="layui-form-label">材质名称：</label>
               <div class="layui-input-block">
                  <input type="text" name="caizhi" lay-verify="required" class="layui-input" id="caizhi">
               </div>
            </div>
            <div class="layui-form-item">
               <label class="layui-form-label">材质描述：</label>
   ...
   <script>
      ...
      layui.use(['table', 'form'], function() {
         ...
         $.ajax({
            type: 'post',
            url: '/XGProject/goodsQuality.do?flag=selectGoodsQuality',
            async: false,
            dataType: 'json',
         ...
         table.on('tool(datatable)', function(obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            //停用操作=============================
            if(layEvent === 'del') { //删除
               layer.confirm('确定停用此材质吗？', function(index) {
                  $.ajax({
                     type: "post",
                     url: "/XGProject/goodsQuality.do?flag=stopGoodsQuality",
                     async: false,
                     data: {
                        "id": data.id
                     },
         ...
         form.on('submit(btn1)', function(obj) {
            var data = obj.field;
            var datas = {
               'goodsQualityId': hangID,
               'goodsQualityName': $('#caizhi1').val(),
               'goodsQualityDefinedTwo': $('#miaoshu1').val(),
               'goodsQualityRemark': $('#remark1').val(),
            };
            var bianji = goodsContent('/XGProject/goodsQuality.do?flag=updateGoodsQuality', datas, 'text');
            if(bianji == 'error') {
               layer.alert('数据上传错误', {
                  icon: 2
               })
               ...
               if(ok != -1) {
                  layer.alert('货物材质修改成功！', {
                     icon: 1
                  }, function() {
                     document.location.reload();
                  });
               } else {
                  layer.alert('货物材质修改失败！', {
                     icon: 5
                  })
               }
            }
         ...
         form.on('submit(btn)', function(obj) {
            ...
            var datas = {
               'goodsQualityName': data.caizhi,
               'goodsQualityDefinedTwo': data.miaoshu,
               'goodsQualityRemark': data.remark,
            }
            layer.confirm('确定添加货物材质吗！', {
               icon: 3
            }, function() {
               var tianjia = goodsContent('/XGProject/goodsQuality.do?flag=addGoodsQuality', datas, 'text');
               if(tianjia == 'error') {
                  layer.alert('数据上传错误！', {
                     icon: 2
                  })
                  ...
                  if(ok != -1) {
                     layer.alert('货物材质添加成功！', {
                        icon: 1
                     }, function() {
                        document.location.reload();
                     });
                  } else {
                     layer.alert('货物材质添加失败！', {
                        icon: 5
                     })
                  }
