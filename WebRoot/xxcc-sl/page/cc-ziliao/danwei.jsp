...
<html>
   ...
   <body>
      <div class="layui-fluid">
         <fieldset class="buttons">
            <legend style="font-size: 1.4rem;">计件单位管理</legend>
            <button class="layui-btn" id="ADD"><i class="fa fa-plus" style="padding-right: 6px;"></i>新增计件单位</button>
         </fieldset>
         <!--表格内容-->
         ...
         <form class="layui-form" action="">
            <div class="layui-form-item">
               ...
               <div class="layui-input-block">
                  <input type="text" name="danwei" lay-verify="required" class="layui-input" id="danwei1">
               </div>
            </div>
            <div class="layui-form-item">
               <label class="layui-form-label">单位描述：</label>
               <div class="layui-input-block">
      ...
      <div id="add" style="display: none;">
         <form class="layui-form" action="">
            <div class="layui-form-item">
               <label class="layui-form-label">单位名称：</label>
               <div class="layui-input-block">
                  <input type="text" name="danwei" lay-verify="required" class="layui-input" id="danwei">
               </div>
            </div>
            <div class="layui-form-item">
               <label class="layui-form-label">单位描述：</label>
   ...
   <script>
      ...
      layui.use(['table', 'form'], function() {
         ...
         $.ajax({
            type: 'post',
            url: '/XGProject/goodsUnit.do?flag=selectGoodsUnit',
            async: false,
            dataType: 'json',
         ...
         table.on('tool(datatable)', function(obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            //停用操作=============================
            if(layEvent === 'del') { //删除
               layer.confirm('确定停用此计件单位吗？', function(index) {
                  $.ajax({
                     type: "post",
                     url: "/XGProject/goodsUnit.do?flag=stopGoodsUnit",
                     async: false,
                     data: {
                        "id": data.id
                     },
         ...
         form.on('submit(btn1)', function(obj) {
            ...
            var datas = {
               'goodsUnitId': hangID,
               'goodsUnitName': data.danwei,
               'goodsUnitDefinedTwo': data.miaoshu,
               'goodsUnitRemark': data.remark,
            };
            var bianji = goodsContent('/XGProject/goodsUnit.do?flag=updateGoodsUnit', datas, 'text');
            if(bianji == 'error') {
               layer.alert('数据上传错误', {
                  icon: 2
               })
               ...
               if(ok != -1) {
                  layer.alert('计件单位修改成功！', {
                     icon: 1
                  }, function() {
                     document.location.reload();
                  });
               } else {
                  layer.alert('计件单位修改失败！', {
                     icon: 5
                  })
               }
            }
         ...
         form.on('submit(btn)', function(obj) {
            ...
            layer.confirm('确定添加库位吗！', {
               ...
               var tianjia = goodsContent('/XGProject/goodsUnit.do?flag=addGoodsUnit', {
                  'goodsUnitName': data.danwei,
                  'goodsUnitDefinedTwo': data.miaoshu,
                  'goodsUnitRemark': data.remark,
               }, 'text');
               if(tianjia == 'error') {
                  ...
                  if(ok != -1) {
                     layer.alert('计件单位添加成功！', {
                        icon: 1
                     }, function() {
                        document.location.reload();
                     });
                  } else {
                     layer.alert('计件单位添加失败！', {
                        icon: 5
                     })
                  }
