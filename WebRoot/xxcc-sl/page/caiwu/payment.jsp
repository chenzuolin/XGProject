	...
	<div class="layui-fluid">
		<fieldset class="buttons">
			...
			<button class="layui-btn" id="ADD">
				<i class="fa fa-plus" style="padding-right: 6px;"></i>新增支付
			</button>
		</fieldset>
		<!--表格内容-->
		...
		<form class="layui-form" action="">
			<div class="layui-form-item">
				...
				<div class="layui-input-block">
					<input type="text" name="payment" lay-verify="required"
						class="layui-input" id="payment1">
				</div>
			</div>
			<div class="layui-form-item">
...
//          //停用操作=============================
//          if(layEvent === 'del') { //删除
//             layer.confirm('确定停用此支付方式吗？', function(index) {
//                $.ajax({
//                   type: "post",
//                   url: "/XGProject/paymentFashion.do?flag=getAll",
//                   async: false,
...
//             //获取对应行的数据
//             hangID = data.id;
//             $("#payment1").val(data.bill);
//             $("#remark1").val(data.remark);
//             openindex = layer.open({
//                type: 1,
//                title: ['编辑支付方式', 'font-size:16px;'],
//                content: $('#edit'),
//                area: '550px',
//                closeBtn: 2,
//                anim: 4,
...
//          var datas = {
//             'pfashionId': hangID,
//             'pfashionName': data.payment,
//             'pfashionRemark': data.remark,
//          };
//          var bianji = goodsContent('/XGProject/paymentFashion.do?flag=updatePaymentFashion', datas, 'text');
//          if(bianji == 'error') {
//             layer.alert('数据上传错误', {
//                icon: 2
//             })
...
//       });
         ...
         form.on('submit(btn)', function(obj) {
            ...
            layer.confirm('确定添加支付方式吗！', {
               icon: 3
            }, function() {
               var tianjia = goodsContent('/XGProject/paymentFashion.do?flag=savePaymentFashion', datas, 'text');
               if(tianjia == 'error') {
                  layer.alert('数据上传错误！', {
                     icon: 2
                  })
                  ...
                  if(ok != -1) {
                     layer.alert('支付方式添加成功！', {
                        icon: 1
                     }, function() {
                        document.location.reload();
                     });
                  } else {
                     layer.alert('支付方式添加失败！', {
                        icon: 5
                     })
                  }
