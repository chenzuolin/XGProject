...
<script>
	function show(str) {
		$(function() {
			...
			$
					.ajax({
						type : "post",
						url : "checks.do?flag=getChecksAll&ff=ajax&pageNow="
								+ now,
						async : true,
						data : {
						...
						success : function(obj) {
							if (obj != null) {
								if (obj[0].result == null) {
									...
									$("#neirong table tbody")
											.append(
													"<tr><td colspan='5' style='text-align:center;'>无订单</td></tr>");
									return false;
								}
								$("#neirong table tbody").html("");
	...
	function tiaozhuanmingxi(str) {
		$(function() {
			...
			document.location.href = "${pageContext.request.contextPath}/checks.do?flag=gePankuMingXi&checkId="
...
<body onload="show('load')">
	<c:if test="${tlist==null }">
		<script>
			document.location.href = "${pageContext.request.contextPath}/tarehouse.do?flag=getTarehouseAll&ff=checks";
		</script>
	</c:if>
