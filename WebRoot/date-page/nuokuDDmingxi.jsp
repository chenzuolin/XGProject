...
<body>
	<%
		...
		x++;
			...
			if(power.get(i).getFunctions().getFunctionName()!=null && power.get(i).getFunctions().getFunctionName().equals("作废挪库订单")){
		pankuzuofei++;
			}
		}
		if(x==0){
	...
	<div class="daohang">
		<ul>
			...
			<a>数据中心</a>
			<span>/</span>&nbsp;
			<a href="javascript:window.history.go(-1)">挪库订单</a>
			<span>/</span>&nbsp;
			<a>查看明细</a>
		</ul>
	</div>
				...
				.click(
						function() {
							if (confirm("你确定要作废此订单吗？")) {
								$("#pankumingxiForm")
										.attr("action",
												"${pageContext.request.contextPath}/shift.do?flag=deleteShift");
