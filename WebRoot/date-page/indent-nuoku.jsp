...
<script>
	function show(str) {
		$(function() {
			...
			$
					.ajax({
						type : "post",
						url : "shift.do?flag=ShiftgetAll&ff=ajax&pageNow="
								+ now,
						async : true,
						data : {
	...
	function tiaozhuanmingxi(str) {
		$(function() {
			...
			document.location.href = "${pageContext.request.contextPath}/shift.do?flag=getNuoKuMingXi&shiftId="
...
<body onload="show('load')">
	<c:if test="${tlist==null }">
		<script>
			document.location.href = "${pageContext.request.contextPath}/tarehouse.do?flag=getTarehouseAll&ff=shift";
		</script>
	</c:if>
