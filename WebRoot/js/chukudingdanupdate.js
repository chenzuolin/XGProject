...
function cangchubianji(str) {
	$(function() {
		...
		var weight = $(str).parents("tr").children("td").eq(8).text();// 取出对应订单中的操作重量
		var number = $(str).parents("tr").children("td").eq(9).text();// 取出对应订单中的操作件数
		var tianche = $(str).parents("tr").children("td").eq(10).text();// 取出对应订单中天车工
		var zhuangxie = $(str).parents("tr").children("td").eq(11).text();// 取出对应订单中的装卸工

		// 判断如果是作废订单，不可以修改
		if ($(str).parents("tr").find("#zhuangtai").val() == "订单作废") {
			alert("作废订单不可以修改！");
		...
		$.ajax({// ajax提交方式
			...
			success : function(obj) {
				...
				if (updateshenpi == "0" && obj[0].result == "no") {
					...
					$("#yscost").attr("disabled", "disabled");
					$("#jiesuan").attr("disabled", "disabled");
					$("#piaoju").attr("disabled", "disabled");
				}
			},
			error : function() {
				$("#cangchuupdate").css("display", "none");
...
function caiwubianji(str) {
	...
	$("#lisuan").attr("disabled", "disabled");
	$("#kuwei").attr("disabled", "disabled");
	$("#picis input[type=checkbox]").attr("disabled", "disabled");
	$("#weight").attr("disabled", "disabled");
	$("#number").attr("disabled", "disabled");
	$("#tianche").attr("disabled", "disabled");
	$("#zhuangxie").attr("disabled", "disabled");
	...
	if ($(str).parents("tr").find("#zhuangtai").val() == "计划出库"
			|| $(str).parents("tr").find("#zhuangtai").val() == "准备出库"
			|| $(str).parents("tr").find("#zhuangtai").val() == "正在出库"
			|| $(str).parents("tr").find("#zhuangtai").val() == "审核未通过"
			|| $(str).parents("tr").find("#zhuangtai").val() == "审核通过"
			|| $(str).parents("tr").find("#zhuangtai").val() == "正在收费") {
		alert("订单不处于收费过程！");
		document.location.reload();
		return false;
	}
